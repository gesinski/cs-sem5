%{
  ...
  int		final[MAXSTATES]; /* final states markedCO with 1's */
...
RE: RE '|' RE {
  ...
  s_new1 = create_state();
  s_new2 = create_state();
  create_transition(s_new1, s1, EPSILON);
  create_transition(s_new1, s3, EPSILON);
  create_transition(s2, s_new2, EPSILON);
  create_transition(s4, s_new2, EPSILON);
  $$ = createRE(s_new1, s_new2);
};

/* Kleene's star */
RE: RE '*' {
  int s1, s2, s_new1, s_new2;
  s1 = FIRST($1);
  s2 = LAST($1);
  s_new1 = create_state();
  s_new2 = create_state();
  create_transition(s_new1, s1, EPSILON);
  create_transition(s2, s_new2, EPSILON);
  create_transition(s_new1, s_new2, EPSILON);
  create_transition(s2, s1, EPSILON);
  $$ = createRE(s_new1, s_new2);
};
